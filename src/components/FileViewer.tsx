...
export function FileViewer({
  ...
  const { lines, loading, error, load, clear } = useFileContent(pod, container);
  const {
    query,
    matches,
    currentMatchIndex,
  ...
  // Content width: total width minus border (2), line numbers, and separator
  ...
  // Map from source line number to first wrapped row index
  ...
  }, [filePath, load, clear, clearSearch]);
  ...
  if (!filePath || loading) {
    return (
      <box flexDirection="column" width="100%" height="100%">
        <box height={1}>
          <text>
            <strong><span fg={theme.accent}>{filePath}</span></strong>
          </text>
        </box>
        ...
        <box height={1}>
          <text>
            <span fg={theme.textDim}>
              ...
              {lines.length}
              {isSearching && ` | n/N: next/prev match | Esc: clear search`}
            </span>
          </text>
        </box>
      )}
